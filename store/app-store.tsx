...
import { AppState, Article, Bookmark, DiaryEntry, LearningTweet, DocumentStoredUpload, Subscription, UsageSummary, UserPreferences, AppUser } from '@/types';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUserData = async (user: AppUser, isInitial: boolean = false) => {
    if (isInitial) {
    ...
    try {
      ...
      // Fetch Subscription
      const { data: subscriptionData } = await supabase.from('subscriptions').select('*').eq('user_id', user.id).maybeSingle();
      ...
      setState(prev => ({
        ...
        articles: articlesData?.map((a: Record<string, unknown>) => ({
          id: a.id as string,
          url: a.url as string,
          title: a.title as string,
          summary: a.summary as string,
          content: a.content as string,
          practiceGuide: a.practice_guide as string,
          status: a.status as Article['status'],
          frequentWords: (a.frequent_words as Article['frequentWords']) || [],
          tags: (a.tags as string[]) || [],
          addedAt: a.added_at as string,
          isTestPassed: a.is_test_passed as boolean | undefined
        })) || [],
        ...
        bookmarks: bookmarksData?.map((b: Record<string, unknown>) => ({
            ...
            addedAt: b.added_at as string
        })) || [],
        documents: documentsData?.map((d: Record<string, unknown>) => ({
            id: d.id as string,
            name: d.name as string,
            type: d.type as 'pdf' | 'markdown' | 'text',
            content: d.content as string,
            summary: d.summary as string,
            keyPoints: (d.key_points as string[]) || [],
            chapters: (d.chapters as DocumentStoredUpload['chapters']) || [],
        ...
        usageSummary: usageData ? {
          totalOperations: (usageData as { total_operations: number }).total_operations,
          totalCostCents: (usageData as { total_cost_cents: number }).total_cost_cents
  ...
  const signInAsGuest = async () => {
    // Cast to AppUser to satisfy type requirements for guest user
    const guestUser = { 
        id: 'guest-user', 
  ...
  const verifyOTP = async (email: string, token: string) => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'OTP検証に失敗しました';
      throw new Error(message);
    }
  };
  ...
  const resendOTP = async (email: string) => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'メール再送信に失敗しました';
      throw new Error(message);
    }
  };
  ...
  const logUsage = async (
    ...
    const inputCostCents = (inputTokens / 1000000) * 0.50 * 150;
  ...
  const deleteAccount = async () => {
    ...
    const confirmed = await showConfirm('本当にアカウントを削除しますか？全てのデータが完全に消去され、復旧することはできません。', 'アカウント削除');
    if (!confirmed) return;
    ...
    try {
        ...
        await supabase.auth.signOut();
        await showAlert('アカウントと全てのデータが削除されました。', 'success');
    } catch (e) {
